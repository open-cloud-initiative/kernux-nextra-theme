...
export interface CommentsGroup {
  ...
  editingComments: Array<Serialized<CommentWithDiscussion>>
}
  ...
  const handleNewCommentCreated = async (comment: Serialized<Comment>) => {
    setComments(prev => {
      ...
      const newGroups = prev.map(group => {
        const index = group.editingComments.findIndex(c => c.gitlabDiscussionId === discussionId)
        if (index !== -1) {
  ...
  return (
    <div className="mt-20 border-t border-gray-300 pt-4">
      <div>
        ...
        {comments.length > 0 && (
          <p className="">
            Die AutorInnen bearbeiten diesen Kommentierungsverlauf in dem openCode Issue: <br />
            <a
          ...
          {user !== null ? (
            <div className="px-8 pb-8 pt-4">
              <div className=" border-t border-gray-300">
                <CommentForm discussionId={null} onCommentCreated={handleNewCommentCreated} />
              </div>
            </div>
          ) : (
            <div className="bg-gray-50">
              <div className="px-4 py-5 sm:p-6">
                <div className="max-w-xl text-sm text-gray-500">
                  <p>
                    Hier können Sie Kommentare zu dieser Seite hinzufügen und an Diskussionen teilnehmen. Diese werden
                    von den AutorInnen in einem Issue auf openCode bearbeitet. Um an der Diskussion teilzunehmen, müssen
                    Sie sich anmelden bzw. registrieren.{' '}
                  </p>
                </div>
