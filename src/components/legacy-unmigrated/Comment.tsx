...
const NameAndDate = ({
  isReply,
  ...comment
}: Serialized<Comment> & { isReply?: boolean }) => {
  return (
    <div className="flex flex-row items-center gap-2">
      {isReply && (
        <ArrowUturnLeftIcon className="text-gray-500 h-4 w-4 rotate-180" />
      )}
      <div className="">
...
const CommentItem: FunctionComponent<{
  ...
  return (
    <div
      id={comment.id}
      className={`${childComment ? "ml-6 -mt-2" : ""} text-base bg-white basis-full`}
    >
      <AlertDialog open={alertOpen} onOpenChange={(open) => setAlertOpen(open)}>
        <AlertDialogContent className="bg-white">
          ...
          <AlertDialogDescription>
            ...
            <br />
            Das Löschen eines Kommentar löscht den jeweiligen{" "}
            <strong>gesamten</strong> Diskussionsverlauf.
          </AlertDialogDescription>
          <AlertDialogFooter>
      ...
      <div className="flex flex-row gap-2">
        <div className="prose flex-1 rounded-sm border p-4">
          <div className="mb-2 flex flex-col sm:flex-row items-start sm:items-center justify-between">
            ...
            {!edit &&
              !comment.authorId?.startsWith("gitlab") &&
              !!currentUserId && (
