...
export const ImplicationGroup: FunctionComponent<{
  ...
  const customImplications = useMemo(
    () => questions.find(q => q.specId === props.spec.specId)?.customSpecImplications || [],
    [questions, props.spec.specId],
  )
  ...
  const handleImplicationToggle = (implication: string) => {
    ...
    const currentSpecIndex = values.findIndex(q => q.specId === props.spec.specId)
    ...
    const implicationIndex = currentImplications.findIndex(imp => imp === implication)
    if (implicationIndex === -1) {
      // add the implication to the list
      values[currentSpecIndex].specImplications = [...currentImplications, implication]

      // set the new values
      form.setValue('questions', values)
    } else {
      // remove the implication from the list
      values[currentSpecIndex].specImplications = currentImplications.filter(imp => imp !== implication)
  ...
  const handleCustomImplicationBlur = (e: React.FocusEvent<HTMLInputElement>, i: number) => {
    ...
    if (i === customImplications.length) {
      ...
      // find the correct implication in the list
      const currentImplications = values[currentSpecIndex].customSpecImplications ?? []

      // add the implication to the list
      values[currentSpecIndex].customSpecImplications = [...currentImplications, value]
      ...
      // add the implication to the list
      values[currentSpecIndex].customSpecImplications = currentImplications.map((imp, index) =>
        index === i ? value : imp,
      )
  ...
  const handleCustomImplicationDelete = (i: number) => {
    ...
    // remove the implication from the list
    values[currentSpecIndex].customSpecImplications = currentImplications.filter((_, index) => index !== i)
        ...
        return (
          <div key={i} className="bg-background rounded-md">
            <Checkbox
              ...
              label={implication}
              checked={Boolean(activeImplications?.some(predicate => predicate === implication))}
            />
          </div>
        )
      })}
      <div className="bg-background rounded-md">
        <div className="space-y-4">
          {customImplications.concat('').map((implication, i) => {
            return (
              <div key={`${i}:${implication}`} className="flex flex-row items-end gap-sm">
            ...
            <div>
              <Button
                ...
                onClick={() => {
                  ...
                  values[currentSpecIndex].customSpecImplications = [...customImplications, '']
