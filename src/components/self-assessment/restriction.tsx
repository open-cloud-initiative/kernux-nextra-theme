...
export default function SelfAssessmentRestriction({form, setActiveStepIndex, specs}: SelfAssessmentRestrictionProps) {
  const sortedSpecs = specs.sort((a, b) => a.specTitle.localeCompare(b.specTitle))
        ...
        <FormProvider {...form}>
          <form>
            <div className="grid grid-cols-6 gap-xl">
              ...
              <div className="col-span-3">
                <Input label="Prüfanwendungsfall" {...form.register('case')} placeholder="Architekturentscheidung" />
              </div>
              <div className="col-span-6" key={'restriction'}>
                <span className="inline-block w-full text-lg font-medium">Vorgaben</span>
                <div className="mt-4">
                  <div>
                    <Popover open={open} onOpenChange={setOpen}>
                      <PopoverTrigger asChild>
                        <Button variant="secondary" className="justify-start" type="button">
                          + Vorgabe hinzufügen
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="p-0 !p-0" side="right" align="start">
                        <Command>
                              ...
                              {sortedSpecs.map(spec => (
                                <CommandItem
                                  ...
                                  onSelect={() => {
                                    ...
                                    let currentSpecs = form.getValues('specifications') || []

                                    // check if the selected spec is already in the list
                                    if (currentSpecs.includes(spec.specId)) {
                                      // remove it
                                      currentSpecs = currentSpecs.filter(specId => specId !== spec.specId)
                                    } else {
                                      // add it
                  ...
                  {selectedSpecIds?.map(specId => (
                    <div key={specId} className="mt-4">
                      <div className="flex flex-row border p-4 items-center rounded-md justify-between gap-2">
                        <span className="font-medium">{specMap[specId].specTitle}</span>
                        <div>
                          <Button
                            variant="secondary"
                            onClick={() => {
