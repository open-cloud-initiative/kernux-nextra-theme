...
const SelfAssessmentDownload: FunctionComponent<{
  ...
  const handleDownload = async () => {
    ...
    try {
      const pdfBlob = await pdfGenerationSvc.generatePDFViaAPI(values, combineAnswers(values, questionFs))
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {error && <p className="text-sm text-red-600">Fehler beim Erstellen des PDFs: {error}</p>}
    </div>
  )
}
