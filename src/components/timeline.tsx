  ...
  const items = useMemo(() => {
    ...
    return sortedItems.map((item, index) => {
      ...
      if (props.oldToNew) {
        ...
        const prevItemYear = index > 0 ? parseDate(sortedItems[index - 1].date).getFullYear() : null
      ...
      const nextItemYear = index < sortedItems.length - 1 ? parseDate(sortedItems[index + 1].date).getFullYear() : null
    ...
    <div className="relative">
      ...
      {items.map((el, i, arr) => (
        ...
        >
          ...
          <div
            className={clsx('absolute rounded-full -left-[29px] md:-left-[2.3rem] bg-white p-1', {
              'top-3': (!el.version && !el.firstOfYear) || (!el.version && el.firstOfYear && !props.oldToNew),
          ...
          <H3>{el.title}</H3>
          {el.date.length > 4 && <Span className="font-bold text-sm">{el.date}</Span>}
          <P
          ...
          {el.firstOfYear && (
            <div
              className={clsx('absolute -left-6 md:-left-[3.75rem] bg-white py-3', {
