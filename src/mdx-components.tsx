...
const A = ({ href = "", ...props }) => (
  <KernUXA href={href} newWindow={EXTERNAL_HREF_REGEX.test(href)} {...props} />
);
...
export const DEFAULT_COMPONENTS: MDXComponents = {
  ...
  blockquote: (props) => (
    <blockquote
      className={cn("text-muted-foreground border-l-2 pl-lg")}
      {...props}
    />
  ),
  hr: (props) => (
  ...
  p: (props) => <P {...props} />,
  tr: ({ className, ...props }) => (
    <tr className={cn("kern-table__row", className)} {...props} />
  ),
  th: ({ className, ...props }) => (
    <th className={cn("kern-table__header", className)} {...props} />
  ),
  thead: ({ className, ...props }) => (
    <thead className={cn("kern-table__head", className)} {...props} />
  ),
  td: ({ className, ...props }) => (
    <td className={cn("kern-table__cell", className)} {...props} />
  ),
  tbody: ({ className, ...props }) => (
    <tbody className={cn("kern-table__body", className)} {...props} />
  ),
        ...
        <div className="flex-1 flex flex-row">
          <div
            className={clsx(
              ...
              config.normalizePagesResult.activeThemeContext.layout === "raw"
                ? ""
                : "gap-xl pt-xl",
              {
                "lg:px-xl ":
                  config.normalizePagesResult.activeThemeContext.layout !==
                  "raw",
              },
            )}
          >
