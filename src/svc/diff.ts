...
async function getDiffContentFromFS(
  diff: string,
  path: string,
): Promise<string> {
...
export const parseDiffContent = async (
  content: string,
): Promise<ParsedDiff[]> => {
...
export function calculateAllowedDiffOptions(
  ...
  const selectedSourceVersion = parseIntoMajorMinorAndPatchVersion(
    selectedSource.name,
  );
  const selectedTargetVersion = parseIntoMajorMinorAndPatchVersion(
    selectedTarget.name,
  );
  ...
  const allowedSource = available.filter((option) => {
    ...
    return (
      isGreaterVersion(selectedTargetVersion, optionVersion) ||
      isEqualVersion(optionVersion, selectedTargetVersion)
    );
  });
  ...
  const allowedTarget = available.filter((option) => {
    const optionVersion = parseIntoMajorMinorAndPatchVersion(option.name);
    return (
      isGreaterVersion(optionVersion, selectedSourceVersion) ||
      isEqualVersion(optionVersion, selectedSourceVersion)
    );
  });
...
export function extractHeadlinesWithState(
...
function determineHeadingType(
  title: string,
): "deleted" | "inserted" | "normal" | "replaced" {
...
function cleanTitle(
  title: string,
  type: "deleted" | "inserted" | "normal" | "replaced",
): string {
  if (type === "replaced") {
...
function addToHierarchy(
  headlines: ParsedDiffHeadings[],
  stack: ParsedDiffHeadings[],
  heading: ParsedDiffHeadings,
) {
