...
export const fetchProtectedResource = async (
  ...
  if (response.status === 401) {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch protected resource after token refresh: ${response.statusText}`,
      );
    }
    return response;
  }
...
export const userInfo = async () => {
  ...
  try {
    const userInfoResponse = await fetchProtectedResource(
      new URL("https://gitlab.opencode.de/oauth/userinfo"),
    );
