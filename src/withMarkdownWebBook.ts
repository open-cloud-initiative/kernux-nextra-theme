...
const transformer = function (ast: any) {
  ...
  const acronymsRegExp = new RegExp(
    "\\b(".concat(Object.keys(acronyms).map(escapeRegExp).join("|"), ")\\b"),
    "g",
  );
      ...
      node.children = node.children
        .map(function (child: any) {
          if (child.type === "text") {
            ...
            const newChildren = child.value.split(acronymsRegExp).map(function (
              part: any,
            ) {
              if (acronyms[part]) {
                return {
...
const buildIndexFiles = () => {
  ...
  for (const file of files) {
    ...
    const matterContent = fileContents.match(
      /^---\s*([\s\S]*?)\s*---\s*([\s\S]*)$/m,
    );
    ...
    const yamlContent = yaml.load(
      (matterContent as RegExpExecArray)[1],
