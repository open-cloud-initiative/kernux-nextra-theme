...
type NormalizedResult = {
  ...
  topLevelNavbarItems: Array<PageItem | MenuItem>
}
      ...
      if (item) {
        ...
        if (typeof window === 'undefined') {
          ...
          for (const [key, value] of Object.entries(item.items)) {
            if (!(value as any).href && children.every((i: any) => i.name !== key)) {
              throw new Error(
...
The field key "${metaKey}" in \`_meta\` file refers to a page that cannot be found, remove this key from "_meta" file.`,
  ...
  for (const currentItem of items) {
    ...
    const title =
      extendedMeta.title ||
      (type !== 'separator' &&
        (currentItem.frontMatter?.sidebarTitle || currentItem.frontMatter?.title || currentItem.name))
    const getItem = () => ({
      ...currentItem,
      type,
      ...extendedMeta,
    ...
    if (normalizedChildren) {
      if (normalizedChildren.activeIndex !== void 0 && normalizedChildren.activeType !== void 0) {
        ...
        activePath = [
          ...
          ...normalizedChildren.activePath.filter((item2: any) => item2.display !== 'children'),
        ]
        switch (activeType) {
          case 'page':
          case 'menu':
